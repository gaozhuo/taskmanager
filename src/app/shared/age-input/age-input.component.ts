...
export class AgeInputComponent implements ControlValueAccessor, OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.form = this.fb.group({
      // 注意：不能直接传initDate，否则报异常：Datepicker: value not recognized as a date object by DateAdapter
      birthday: [new Date(initDate), this.validateDate],
      age: this.fb.group({
        ageNum: [initAge.age],
    ...
    this.subBirth = merged$.subscribe(date => {
      ...
      if (date.from === 'birthday') {
        ...
        if (age.age !== ageToCompare.age || age.unit !== ageToCompare.unit) {
          this.form.get('birthday').patchValue(new Date(date.date), {emitEvent: false});
          this.propagateChange(date.date);
        }
      }
    });
